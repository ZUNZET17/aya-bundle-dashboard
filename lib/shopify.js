...
export async function shopifyGqlRequest({ query, variables, storeData }) {
  console.log(storeData.storefront_env, 'store', storeData)
  console.log('process', process.env.NEXT_PUBLIC_STOREFRONT_API_URL_AOA_US)
  const options = {
    method: "POST",
    headers: {
...
export async function shopifyAdminGqlRequest({ query, variables, store }) {

  const response = await axios({
    url: store.admin_env,
    method: 'post',
    headers: {
      "X-Shopify-Access-Token": store.admin_token_env,
      "Accept": "application/json",
      "Content-Type": "application/json",
    },
    data: {
...
export async function loadProducts(ids, store) {
  ...
  const response = await shopifyAdminGqlRequest({query}, store);
